<h1>Emanuel Ferreira <sub><sup>aka Holmes</sup></sub></h1>

Engenharia de Software (UCSal, 5° semestre) · IT Intern @ Continental AG · Tech Lead @ Projeto Vox

## Sobre mim

Atuo na Continental AG no Service Desk corporativo — gestão de identidades e acessos (IAM) via Active Directory, padronização de estações de trabalho e gestão de incidentes sob SLAs rigorosos.

No Projeto Vox (open-source), sou Tech Lead e responsável pela arquitetura ponta a ponta: banco vetorial, pipeline RAG, integração de APIs e esteiras de CI/CD.

Fora do código, participo ativamente de comunidades de tecnologia. Palestrei no [NOSS 2026](https://github.com/cumbucadev/NOSS), fui voluntário em um evento de hacking e estou sempre em algum evento de IA ou open source. Tudo feito da comunidade pra comunidade faz meu coração bater mais forte.

**Interesses:** Engenharia de Dados · RAG & GenAI · Segurança da Informação (IAM/Blue Team)

---

## Projeto Vox AI 🏳️‍🌈

Assistente de IA com busca semântica (RAG) voltado à comunidade LGBTQIA+.

| Problema | Solução | Resultado |
| :--- | :--- | :--- |
| MVP sem escala (Google Sheets/JSON) | Migração para Supabase (PostgreSQL + pgvector) | Arquitetura escalável com RLS |
| Inferência local pesada (Sentence Transformers) | Substituição por APIs externas | ~80% de redução no uso de RAM |
| Sem auditoria de código | CI/CD com Gatekeeper + AI Code Review via GitHub Actions | PRs validados automaticamente |

[Repositório](https://github.com/emn-f/vox-ai) · [Deploy](https://vox-ai.streamlit.app/)

---

## Tech Stack

**Dados, Backend & IA**
<div align="left">
  <img src="https://skillicons.dev/icons?i=python,postgres,mysql,supabase,java&perline=10" />
</div>

**Infra & DevOps**
<div align="left">
  <img src="https://skillicons.dev/icons?i=linux,githubactions,git&perline=10" />
</div>

---

## Publicações no Medium
Compartilho um pouco do que aprendo de vez em quando :)
- [Do MVP ao RAG: Como o Vox AI evoluiu de planilhas para uma arquitetura escalável com Supabase](https://emnferreira.medium.com/do-mvp-ao-rag-como-o-vox-ai-evoluiu-de-planilhas-para-uma-arquitetura-escal%C3%A1vel-com-supabase-4c51b951ed56)

---

## Contato

<div>
  
[![Gmail](https://img.shields.io/badge/-Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:emnferreira.wk@gmail.com)
[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/emanuelferreira)
[![Medium](https://img.shields.io/badge/-Medium-12100E?style=for-the-badge&logo=medium&logoColor=white)](https://medium.com/@emnarlanferreira)
[![Lattes](https://img.shields.io/badge/-Lattes-1A73E8?style=for-the-badge&logo=google-scholar&logoColor=white)](http://lattes.cnpq.br/1681360365008995)
</div>

---

<div align="center"> 
  <img src="https://raw.githubusercontent.com/platane/platane/output/github-contribution-grid-snake.svg?user=emn-f" alt="Snake animation" /> 
</div>


